...
export default function MapView({ onReportHere }) {
  ...
  useEffect(() => {
    ...
    SHELTER_SEED.forEach((shelter) => {
      L.marker([lat + shelter.latOffset, lng + shelter.lngOffset], {
        icon: shelterIcon(),
      })
        .addTo(map)
    ...
    villages.forEach((village) => {
      ...
      marker.on("popupopen", (event) => {
        ...
        popup
          ...
          ?.addEventListener("click", () => onReportHere(village.id));
      });
      ...
      L.circle(position, {
        ...
        fill: false,
        opacity:
          showRiskLayer && (state === "NORMAL" || state === "WATCH") ? 0.9 : 0,
      }).addTo(map);
    });
